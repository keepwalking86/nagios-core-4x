...
##
## Created : <2014-10-25>
## Updated: Time-stamp: <2014-10-26 23:14:38>
##-------------------------------------------------------------------
if [ "$1" = "-w" ] && [ "$2" -gt "0" ] && \
    [ "$3" = "-c" ] && [ "$4" -gt "0" ]; then
    ...
    if [ "$memVmRSS" -ge "$4" ]; then
        echo "Memory: CRITICAL VIRT: $memVmSize MB - RES: $memVmRSS MB used!|VIRT=$(($memVmSize*1024*1024));;;; RES=$(($memVmRSS*1024*1024));;;;"
        $(exit 2)
    elif [ "$memVmRSS" -ge "$2" ]; then
        echo "Memory: WARNING VIRT: $memVmSize MB - RES: $memVmRSS MB used!|VIRT=$(($memVmSize*1024*1024));;;; RES=$(($memVmRSS*1024*1024));;;;"
        $(exit 1)
    else
        echo "Memory: OK VIRT: $memVmSize MB - RES: $memVmRSS MB used!|VIRT=$(($memVmSize*1024*1024));;;; RES=$(($memVmRSS*1024*1024));;;;"
        $(exit 0)
    fi
    ...
    echo ""
    echo "Usage:"
    echo "check_mem.sh -w <warn_MB> -c <criti_MB> <pid_pattern> <pattern_argument>"
    echo ""
    echo "Below: If tomcat use more than 1024MB resident memory, send warning"
    echo "check_mem.sh -w 1024 -c 2048 --pidfile /var/run/tomcat7.pid"
    echo "check_mem.sh -w 1024 -c 2048 --pid 11325"
    echo "check_mem.sh -w 1024 -c 2048 --cmdpattern \"tomcat7.*java.*Dcom\""
    echo ""
